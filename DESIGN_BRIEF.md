...
5. **Re-roll card numbers (1-5)** — confirmed by the owner: the printed number is mechanical, not cosmetic. Each re-roll card grants that many total re-roll attempts when played (e.g. a "Press Pass 3" lets you roll the die up to 3 times total, choosing after each roll whether to keep the result or roll again, keeping the final value). Mesmera the Veiled adds 3 more attempts on top of whatever the card itself grants.
...
Doesn't need to be optimal, just legal and coherent. Reasonable heuristics: prefer free draft picks that fill an empty mat slot over ones that would bump a card to reserve; weight Performers by matching resource type to whichever resource the bot is behind on; use Favor cards for extra turns when available rather than hoarding (and may spend more than one eligible Favor before its main action in the same turn window); spend re-roll cards defensively when a Tomato die would hit a nearly-empty-hearts card the bot cares about; otherwise acquire the highest-value card available for free before considering a market purchase. The engine itself resolves bot decisions instantly (no artificial "thinking" delay) so games never stall — but per the owner's request, the server paces the *broadcast* of a bot's moves, waiting 2 seconds after a bot fully completes a turn before letting the next bot act, so human spectators/players can actually follow what each AI just did.
