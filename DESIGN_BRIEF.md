...
- 139 cards: 64 Performers, 10 Backdrop, 10 Prop, 10 Trainer, 5 Re-roll (Press Pass 1-5 — the Audience/"Tomato" re-roll cards were removed from the game per a later ruling), 16 Favor (8x "1st", 8x "2nd+"), 24 Resource (1/2/3, x2 each, of Hearts/Stars/Coins/Cards).
...
- Shuffle the 139-card deck, place it centrally as the draw pile.
...
3. **Dice phase:**
   ...
   - **Roll Tomato dice, as one batch.** Roll a number of D8 Tomato dice equal to the current round number (round 1 = 1 die, round 2 = 2 dice, etc), all at once, and leave the whole batch visible before it locks. Between the roll and the lock, if the round's Mesmera the Veiled holder wants to, they may re-roll the entire batch once (see the Trainer table below). Once locked, for every rolled number 1-8, every player removes 1 heart from whatever currently occupies that slot number on their own mat. Every card also has one hidden "built-in" heart beyond what's printed on its face (per the owner's ruling): a hit while the card has 1+ printed hearts left just removes a printed heart and the card survives, same as before. A hit taken while the card is already at 0 printed hearts is absorbed by the hidden heart the first time (the card survives, clinging on with 0 visible hearts) — only a *second* hit taken while already at 0 actually discards the card. This applies to both Tomato-die hits and trophy-winner heart removal above.
...
- **Backdrop/Prop** (10 unique effects each, one copy of each = 20 cards): occupy the Backdrop or Prop mat slot. See `card_database.json` for each card's `boostKind` (characteristic or type), `boosts` (which value(s) it matches — the 2 wildcard cards per set match all 4 characteristics or all 4 types), `startingHearts` (2 for the 8 single-match cards, 1 for the 2 wildcard cards — how many hearts it holds when acquired), and `maxHearts` (3 for every Backdrop/Prop — its total heart capacity, matching the printed card's full+empty heart icons; a solid card can gain 1 more heart, a wildcard card can gain 2 more, via Heart resource cards or Collection Die heart payouts). Effect (see "Rules the written doc left open" below for sourcing): when a Collection Die causes a Performer on the same player's board to collect, and that Performer's characteristic/type matches the player's equipped Backdrop or Prop, that performer collects +1 extra unit of their printed resource.
...
- **Favor cards** (16 = 2 unique x8): go to reserve on acquire. Timing, per the owner's exact ruling: a "1st" Favor is only usable on the player's actual first turn of the round; a "2nd" Favor is usable on their second turn or any later turn that round (i.e. once `p.turns` — turns already completed this round — is 0 for "1st", or >=1 for "2nd"). While eligible, the card can be discarded from reserve at any point during one of your own turns, before you take that turn's main action, for an extra turn right then. A player may spend more than one eligible Favor this way before their main action in the same turn window — each spent Favor queues its own extra turn, taken in order once the current turn's main action resolves. There is never a forced prompt for this — the owner was explicit that the game must never ask "do you want to use a Favor?"; the card just becomes clickable in the player's own reserve once eligible, and not clicking it simply means play continues as normal (`useFavor` action, separate from the turn's main action). Rule: you can't spend a Favor draft pick to draft another Favor of the exact same timing (1st-for-1st or 2nd-for-2nd) — i.e. a bonus turn granted by a Favor may not be used to draft another same-timing Favor; normal acquisition (paying coins or drafting on a normal turn) is unaffected.
- **Re-roll cards** (5 = Press Pass 1-5; the Audience/"Tomato" re-roll cards were removed from the game per a later ruling): go to reserve on acquire. Unlike a normal reserve card, each Press Pass is positional and single-use per game: "Press Pass N" can only be spent while the round's Nth Collection Die (in roll order) is open — rolled but not yet locked — and it re-rolls that die up to N times (keeping the final result) before being discarded. There is no forced prompt; the card simply becomes clickable in reserve during that die's open window, for whichever one player happens to hold the matching card, and not clicking it just means the die locks in as rolled.

### Trainer abilities (implement each as an isolated, clearly-named rule module)

| Trainer | Ability |
|---|---|
| Madame Coeur | Any card you draft or place onto your board starts at its printed maximum heart count instead of its normal starting-heart value. (Does not change the card's printed capacity — just its starting fill.) |
| Tomasso the Terrible | Discard 1 card from the draft row to trigger an extra Tomato die roll immediately; the resulting heart loss applies to every OTHER player's board, not yours. Per the owner's ruling, using this ability consumes the player's entire turn — same as any other main action, no drafting/buying/rearranging afterward. |
| Madame Curio | Discard 1 card from the draft row to trigger an extra Collection Die roll immediately; only you collect from it, other players' matching performers do not. Consumes the player's entire turn, same as Tomasso. |
| Professor Stainglass | Discard 1 card from the draft row to draw the top card of the deck directly into your reserve. Consumes the player's entire turn, same as Tomasso. |
...
| Mesmera the Veiled | After the round's Tomato dice batch is rolled (but before it locks in), you may choose to re-roll the entire batch one time, keeping the new results. |
...
5. **Re-roll card numbers (1-5)** — superseded by a later, more specific ruling (see the "Re-roll cards" entry above): the printed number identifies *which* Collection Die of the round the card can react to (Press Pass 1 → the round's 1st die, ... Press Pass 5 → the 5th), and grants that many re-rolls of that one die. The Audience/"Tomato" re-roll cards were removed from the game entirely, and Mesmera the Veiled's ability was changed to a one-time re-roll of the whole Tomato batch rather than adding extra re-roll attempts to these cards.
...
`assets/card_database.json` has six top-level arrays: `performers` (64), `propsAndBackdrops` (20), `trainers` (10), `resources` (24), `favors` (16), `rerolls` (5). Every card object has an `image` field with a path relative to `assets/` — every path has been verified to resolve to a real file, so load directly, no name-guessing needed.

## AI bot behavior guidance

Doesn't need to be optimal, just legal and coherent. Reasonable heuristics: prefer free draft picks that fill an empty mat slot over ones that would bump a card to reserve; weight Performers by matching resource type to whichever resource the bot is behind on; use Favor cards for extra turns when available rather than hoarding (and may spend more than one eligible Favor before its main action in the same turn window); spend a Press Pass defensively when its matching Collection Die is a dead roll for the bot and its board has enough letter variety that a re-roll probably helps; use Mesmera's Tomato-batch re-roll only when the current batch threatens one of the bot's own cards; otherwise acquire the highest-value card available for free before considering a market purchase. The engine itself resolves bot decisions instantly (no artificial "thinking" delay) so games never stall — but per the owner's request, the server paces the *broadcast* of a bot's moves: 2 seconds after a bot fully completes a turn before the next bot acts, and (during the dice phase) each Collection Die and the Tomato batch is held on screen briefly — long enough for any bot's Press Pass/Mesmera reaction to resolve first — before locking in and moving to the next step, so human spectators/players can actually watch the round unfold.
